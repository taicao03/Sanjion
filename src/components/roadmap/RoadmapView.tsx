...
export function RoadmapView({
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fadeIn font-mono">
      {/* Hero Banner Header */}
      <div className="relative overflow-hidden rounded-lg bg-[#181F2A] border border-slate-700/60 p-6 md:p-8 text-white shadow-md">
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded bg-[#0F141C] border border-slate-700/60 text-xs font-bold text-[#C9962C]">
              <Sparkles className="w-3.5 h-3.5 text-[#C9962C]" /> Lộ Trình Luyện
              Tập Senior Roadmap (A -&gt; Z)
            </div>
            <h1 className="text-2xl md:text-3xl font-display font-bold text-white tracking-tight">
              Hành Trình Chinh Phục
            </h1>
            <p className="text-slate-200 text-xs md:text-sm leading-relaxed">
              Theo dõi và hoàn thành từng bài học với sự trợ giúp của AI Tutor.
            </p>
          </div>

          {/* User Level & Completion Card */}
          <div className="bg-[#0F141C] border border-slate-700/60 rounded p-4 w-full md:w-80 flex flex-col gap-2.5 text-white">
            ...
            <div className="flex items-center justify-between text-xs text-slate-300 pt-1 border-t border-slate-700/60">
              <span className="flex items-center gap-1 text-[#C9962C] font-bold">
                <Flame className="w-3.5 h-3.5 text-[#C9962C]" /> Streak:{" "}
                <b>{profile.streakCount}d</b>
              </span>
              <span className="flex items-center gap-1 text-[#C9962C] font-bold">
                <Star className="w-3.5 h-3.5 text-[#C9962C]" /> XP:{" "}
                <b>+{profile.totalPoints}</b>
              </span>
            </div>
          </div>
        </div>
          ...
          return (
            <button
              ...
              className={`relative overflow-hidden rounded-lg p-4 text-left transition-colors duration-150 border ${
                isActive
                  ? "bg-[#181F2A] border-[#C9962C] text-white shadow-md"
                  : "bg-[#181F2A] border-slate-700/60 text-slate-300 hover:border-slate-500 hover:text-white"
              }`}
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2.5">
                  <div className="p-2 rounded bg-[#0F141C] border border-slate-700/60">
              ...
              <div className="space-y-1">
                ...
                <div className="w-full bg-[#0F141C] rounded-sm h-1.5 overflow-hidden">
                  <div
      ...
      {(() => {
        ...
        return (
          <div className="bg-[#181F2A] rounded-lg border border-slate-700/60 p-6 space-y-6 shadow-md">
            {/* Stage Header */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-700/60 pb-5">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="px-2.5 py-0.5 rounded text-xs font-bold bg-[#0F141C] text-[#C9962C] border border-[#C9962C]/40">
              ...
              <div className="flex flex-wrap gap-1.5 max-w-md">
                {currentStage.targetSkills.map((skill, idx) => (
                  <span
                    key={idx}
                    className="text-[11px] bg-[#0F141C] text-slate-200 px-2 py-0.5 rounded border border-slate-700/60"
                  >
                    🎯 {skill}
                  </span>
                ))}
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                <Target className="w-4 h-4 text-[#C9962C]" /> Danh Sách Bài Tập
                Theo Tiến Độ
              </h3>
                  ...
                  return (
                    <div
                      ...
                      className={`group relative overflow-hidden rounded border p-4 transition-colors duration-150 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 ${
                        isSolved
                          ? "bg-[#0F141C] border-[#2FAE79]/40"
                          : "bg-[#0F141C] border-slate-700/60 hover:border-slate-500"
                      }`}
                    >
                      ...
                      <div className="flex items-start gap-3 flex-1">
                        <div
                          className={`w-8 h-8 rounded flex items-center justify-center font-bold text-xs flex-shrink-0 ${
                            isSolved
                              ...
                              : "bg-[#181F2A] text-[#C9962C] border border-slate-700/60"
                          }`}
                        >
                          {isSolved ? (
                            <CheckCircle2 className="w-4 h-4 text-[#2FAE79]" />
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center flex-wrap gap-2 text-[11px]">
                            ...
                            <span className="px-2 py-0.5 rounded bg-[#181F2A] text-slate-200 border border-slate-700/60 font-bold">
                              {node.difficulty}
                            </span>
                          ...
                          <h4 className="text-sm font-sans font-bold text-white group-hover:text-[#C9962C] transition-colors">
                            {node.title}
                          </h4>
                        ...
                        {targetQuestion ? (
                          <button
                            ...
                            className={`w-full md:w-auto inline-flex items-center justify-center gap-2 px-4 py-1.5 rounded font-bold text-xs transition-colors cursor-pointer ${
                              isSolved
                                ? "bg-[#181F2A] text-slate-200 border border-slate-700/60 hover:text-white"
                                : "bg-[#2FAE79] hover:bg-[#2FAE79]/90 text-slate-950 font-bold"
                            }`}
                          >
                            {isSolved ? (
                              <>
                          ...
                          <button
                            disabled
                            className="inline-flex items-center gap-2 px-3 py-1.5 rounded bg-[#181F2A] text-slate-400 text-xs border border-slate-700/60 cursor-not-allowed"
                          >
            ...
            {/* AI Assistant Banner */}
            <div className="bg-[#0F141C] border border-slate-700/60 rounded-lg p-5 flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="p-2.5 bg-[#5B54D9]/20 text-[#5B54D9] rounded border border-[#5B54D9]/40">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-300">
                    Trò chuyện cùng AI Sanjioner để giải đáp thắc mắc và gợi ý
                    hướng đi.
                  </p>
                </div>
              </div>
              {onOpenAiAssistant && (
                <button
                  ...
                  className="px-3 py-1.5 rounded bg-[#5B54D9]/20 hover:bg-[#5B54D9]/30 border border-[#5B54D9]/40 text-white font-bold text-xs whitespace-nowrap transition-colors cursor-pointer"
                >
                  💬 Hỏi AI Sanjioner
                </button>
              )}
